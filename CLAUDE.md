...
- **Phase 1C (Resilience, Data Safety, UX Polish)** ✅ completed (apr 2026) — 14 задач robustness + visibility:
  - **Content Validation & Robustness**: `src/v2/content/validate.ts` runtime validator for all cross-references (choice.next, node.next, battle.encounterId, loot.itemDefId, location dialogue ids) — throws at `registerV2Scenes` on boot before any scene loads. DialogueRunner `applyEffects` wraps each effect in its own try/catch with `contentError` emit (partial-apply documented Phase 1C behaviour, NOT transactional — DECISIONS R1). DialogueScene empty-choices fallback → «← Вернуться в Hub» via `sceneRouter.setRoot`. CombatBridgeScene idempotency guard (`resultApplied` flag) + missing-encounter synthetic defeat that bypasses applyResult and toasts on destination (R3).
  - **Data Safety**: SaveManager hardened — `save()` catches QuotaExceededError, sets `saveFailed` flag, emits `eventBus.emit("saveError", ...)`. `importJson` returns discriminated union `{ ok: true } | { ok: false, error }`, runs shallow shape validation + orphan equipped cleanup + `lastSavedAt` future-clamp before module-level `migrate()` in its own try/catch. InventorySystem adds `removeItem(id): boolean` with atomic equipped-slot cleanup, extracts module-level `SLOT_ORDER` (R7). RelationshipSystem `logDecision` adds 30-day age-trim (order: age-trim → push → count-trim) with defensive `?? []` guard.
  - **UX & Visibility**: New `src/v2/ui/Toast.ts` — scene-bound non-blocking notification (depth 2000, scene-local tweens, `WeakMap<Scene, LiveToast[]>` for auto-stacking and SHUTDOWN cleanup, R4). Toast wired once at boot in `src/v2/index.ts` `wireToastSubscriptions(game)` with module-level `toastWired` idempotency flag (RISK-1). HubScene XP bar under greeting + location background asset fallback. PlayerStatsScene backpack pagination for ≥5 items. LocationScene asset fallback for missing background (dark purple + placeholder text + in-place toast). PostCombatScene resilient to missing character/encounter (`synthesizedDefeat` flag path).
  - **EventBus extended** with `saveError`, `contentError`, `assetError` typed payloads — single locked shape per DECISIONS §2, first-claimer owned the edit.
  - Все 14 задач завершены, все code reviews approved (architect-frontend + architect-backend + architect-systems). v1 chunk 132.77 kB (≤135 kB budget), v2 chunk **87.20 kB (within the Phase 1C temporary 90 kB ceiling — original Phase 1B budget was 80 kB, raised to 85 kB by architects consensus per DECISIONS §1, then bumped to 90 kB by tech-lead after PlayerStatsScene pagination measurement). Phase 2 commitment: revert to 80 kB (delta −7.20 kB) via dedup of repeated string constants across scenes, DEV-only validator gating per R11, and Toast alpha-lerp instead of tween config blob.**
- **Phase 2A (Arena Archero-mode + Items expansion + Magazin)** ✅ completed (apr 2026) — 19 задач:
  - **Arena (Archero-style roguelike)**: новая ветка из HubScene «⚔️ Арена» → ArenaScene (entry menu) → ArenaRunScene (active run) → ArenaRewardScene (выбор бафа между боями) → CombatBridgeScene (использует синтетический EncounterDef через `arenaEncounterGenerator.generate()` — fallback chain: ENCOUNTERS → generator → handleMissingEncounter). 5 обычных боёв + 1 финальный босс на 6-м этаже = 6 encounters. Permadeath: проигрыш сбрасывает run, persistent XP/gold/items сохраняются. `ArenaSystem.completeRun/abortRun` вызывают `gameState.flush()` для защиты от Telegram WebView background-suspend.
  - **BuffSystem**: 10 run-only buff defs в `src/v2/content/buffs/index.ts`. Полностью реализованы 5 stat-only (addPhysAttack/Mag/Hp/Mp/Crit + physPerFightSurvived). Применяются через `buffSystem.applyToStats()` хук в `EncounterBuilder.build()` — non-arena fights не затронуты (early return при `activeRun === null`). Сбрасываются при завершении run. extraReward буфы расширяют выбор в ArenaRewardScene с 3 до N карт.
  - **Items expansion**: 6 → 24 предмета в `src/v2/content/items/index.ts`. Существующие 6 не тронуты. Добавлены 18 новых: 6 weapons + 6 armor + 6 accessories. ItemRarity union расширен `"legendary"` тиром. Все 6 legendary предметов содержат `crit` поле. Stat scaling: common (baseline) → rare (+50%) → epic (+120%) → legendary (+250%). PlayerStatsScene показывает rarity colors на equipment + backpack rows (gold/purple/blue/gray).
  - **ShopSystem + ShopScene**: новая кнопка «🛒 Магазин» в Hub → ShopScene показывает 6 ротирующихся товаров (deterministic-per-day seed). Цены по rarity: 50/150/400/1000 gold. Legendary gated to level 5+, epic to level 3+. Покупка → toast feedback (Куплено / Недостаточно золота / Рюкзак переполнен).
  - **5 синтетических arena enemies** в `src/v2/content/characters/arena-enemies.ts` (Бандит / Тёмный маг / Страж / Отступник / Демон). Каждый с уникальным `bossPattern`. CharacterPortrait fallback на placeholder Arc + initial.
...
- **Phase 2B — arena-rework-v2** ✅ completed (apr 2026) — вторая feature Phase 2B. 10 задач (14 merged):
  - **Arena rework (10 bosses)**: ArenaEncounterGenerator расширен с 6 до 10 боёв (9 normal + 1 final boss на floor 10). Authored BOSS_CURVE table с 49 total HP layers. Per-run difficulty scaling `1.15^totalRunsCompleted` applied to HP + attack (NOT layers). Buff milestones at floors 3/6/9. `ARENA_TOTAL_FLOORS = 10`, `BUFF_FLOORS = [3, 6, 9]`.
  - **Perk system (Option B)**: `ArenaPerkApplicator.ts` (pure derived view — `computeEffectiveSkills` + `computePassiveSnapshot` with 14 effect fields). `ArenaPerkSystem.ts` (singleton — `initForRun`/`getCardOptions`/`applyCard`). ZERO SKILL_CONFIG mutation — reads `SKILL_LEVEL_TABLE` (exported from v1 PerkManager, one-line change) and computes locally. Three-tier card generation: skill → passive → stat safety net. Guaranteed 3 cards always.
  - **Perk data**: 12 passive perks (`content/perks/passive-perks.ts`) + 6 stat perks (`content/perks/stat-perks.ts`) + type definitions (`content/perks/types.ts`) + barrel export (`content/perks/index.ts`). Pure data, zero Phaser imports.
  - **Passive effects**: `PassivePerkEffects.ts` — 12 pure query functions (getLifestealPercent, hasDefuser, shouldIgnoreShield, getRageDamageMultiplier, etc.). All self-gate on activeRun state. 3 deferred (bomb_master, explosive_magic, crit_mastery — runtime hooks needed in GameScene, Phase 2C).
  - **Stat perks in EncounterBuilder**: `arenaStatPerkSystem.ts` mirrors BuffSystem.applyToStats pattern. Hooked AFTER buffSystem.applyToStats in EncounterBuilder.build(). `PlayerCombatStats.manaStart?: number` added for stat_start_mp + mana_surge passive.
  - **Greeting phrases**: `CharacterDef.greetings?: string[]` (optional, backward-compat). 5 arena enemies x 3 phrases = 15 authored. BossTransition animation CUT (R2A-7), greetings available for CombatBridgeScene overlay in future pass.
  - **Sell items**: PlayerStatsScene sell icon + SellConfirmModal + InventorySystem.removeItemAndRefund.
  - Budget: v2 chunk **134.33 kB**, v1 chunk **135.46 kB**. Phase 2C revert commitment to 90 kB via SceneChrome.ts + theme.ts + modalChrome.ts extraction + Russian string dedup. DECISIONS R2A-1..R2A-8 documented in `.claude/teams/feature-arena-rework-v2/DECISIONS.md`.
- **Phase 2C** — не начаты.
